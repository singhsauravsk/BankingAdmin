...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			request.getRequestDispatcher("/WEB-INF/views/successLogin.jsp").forward(request, response);
		} catch(LoginException le) {
			request.setAttribute("message", le.getMessage());
			request.getRequestDispatcher("/WEB-INF/views/failLogin.jsp").forward(request, response);
		}
	}
}
